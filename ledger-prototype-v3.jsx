...
const DEFAULT_CRYPTO_LABELS = [
  ...
  { id: "sl_a2", label: "Client crypto withdrawal", status: "active", legs: [{ accountId: "acc_210302", side: "debit", pct: 100 }, { accountId: "acc_113102", side: "credit", pct: 100, wallet: true }, { accountId: "acc_140101", side: "debit", pct: 1.785714 }, { accountId: "acc_410101", side: "credit", pct: 1.785714 }] },
...
const TRANSLATIONS = {
  en: {
    ...
    nav_coins: "Coins", nav_wallets: "Wallets", nav_cryptoTx: "Crypto Transactions", nav_revaluation: "Revaluation", nav_types: "Label Mapping", nav_gasTanks: "Gas Tanks",
  ...
  th: {
    ...
    nav_coins: "เหรียญ", nav_wallets: "กระเป๋าเงิน", nav_cryptoTx: "ธุรกรรมคริปโต", nav_revaluation: "ตีราคาใหม่", nav_types: "การจับคู่ป้ายกำกับ", nav_gasTanks: "แท็งก์ค่าแก๊ส",
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: "gasTanks", labelKey: "nav_gasTanks", icon: Zap },
...
function TenantWorkspace({ tenantId, tenantName, tenants, seed, hasCrypto, onSwitchTenant, onManageTenants }) {
  ...
  const [explorerKeys, setExplorerKeys] = useState({ etherscan: "", tronscan: "", coingecko: "" });
  ...
  const CRYPTO_TABS = ["coins", "wallets", "cryptoTx", "gasTanks", "revaluation"];
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (saved) {
        ...
        setCryptoLabels((saved.cryptoLabels || seedData.cryptoLabels || []).map(normalizeCryptoLabel));
  ...
  useEffect(() => {
    try {
      ...
      setExplorerKeys(parsed && typeof parsed === "object"
        ? { etherscan: parsed.etherscan || "", tronscan: parsed.tronscan || "", coingecko: parsed.coingecko || "" }
        : { etherscan: "", tronscan: "", coingecko: "" });
    } catch { /* ignore */ }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tenantId]);
  useEffect(() => {
...
function CryptoLabelManager({ accounts, cryptoLabels, onAdd, onUpdate, onDelete }) {
  ...
  function startEdit(l) { setEditingId(l.id); setEditForm({ label: l.label, kind: l.kind, fullEntry: l.fullEntry, appliesTo: l.appliesTo, legs: (l.legs || []).map((x) => ({ accountId: x.accountId, side: x.side, pct: x.pct, wallet: x.wallet })) }); }
  function saveEdit() {
    if (!valid(editForm)) return;
    onUpdate(editingId, { label: editForm.label.trim(), legs: cleanLegs(editForm.legs), ...(editForm.kind ? { kind: editForm.kind } : {}), ...(editForm.fullEntry ? { fullEntry: true, appliesTo: editForm.appliesTo } : {}) });
            ...
            {shownLabels.map((l) => (
              editingId === l.id ? (
                ...
                <tr key={l.id} className="border-b border-stone-100 last:border-0">
                  ...
                  <td className="px-3 py-2 text-slate-500 text-xs align-top">{legsText(l.legs)}</td>
...
function CryptoTransactions({ cryptoTxs, coins, wallets, accounts, cryptoLedger, cryptoRules, cryptoLabels = [], cryptoFilters = [], onAddFilter, onDeleteFilter, walletLabelRules, onAdd, onUpdate, onDelete, onBuildJournals, onLoadSample, onImportCsv, onPostOne, onBulkPost, onBulkLabelPost, onRemember, onRememberLabel }) {
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div className="flex gap-1 mb-4 border-b border-stone-200">
        {[["import", "Import"], ["transactions", "Transactions"], ["rollforward", "Asset Roll Forward"], ["layers", "Unused Cost Layers"]].map(([k, l]) => (
          <button key={k} onClick={() => setSubTab(k)}
            className={`px-3 py-2 text-sm border-b-2 -mb-px ${subTab === k ? "border-[#03D47C] text-[#02B169] font-medium" : "border-transparent text-slate-500 hover:text-slate-700"}`}>
            {l}
          </button>
